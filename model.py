...
class DenoisingModule(nn.Module):
    def __init__(self, input_size, input_ch,
        ...
        super().__init__()
        # DEBUG, comment these related to 'self.noise_pred' to save memory
...
class DenoisingNet(nn.Module):
    def __init__(self, T, net_type, cfg, cfg_ratio, input_size=32, input_ch=3, beta_1=0.0001, beta_T=0.02, tau_S=-1):
        super().__init__()
        ...
        for idx, t in enumerate(self.t_series):
            ...
            layer = DenoisingModule(input_size, input_ch, net_type, layer_cfg,
                                    alpha, alpha_bar, alpha_bar_prev,
...
if __name__ == '__main__':
    ...
    cfg_ratio = [1.0]
    model = DenoisingNet(1000, net_type, cfg, cfg_ratio, tau_S=20).to(device).eval()
    # print(model)
    ...
    # from network.unet_time import UNet_Conditional_Time
    ...
    # print(json.dumps({'net_type': net_type, 'cfg': cfg, 'cfg_ratio': cfg_ratio}, indent=4))
